<?php
    ...
    $result = $_SESSION['result']['nameBook'];

    $id_search_book = mysqli_query($connect, "SELECT * FROM `book` WHERE `nameBook` = '$result'");

    if(mysqli_num_rows($id_search_book) > 0) {
        while ($book = mysqli_fetch_array($id_search_book)) {
            echo
